...
router.put('/availability', protect, roleGuard('hamali'), asyncHandler(async (req, res, next) => {
  ...
  let profile = await HamaliProfile.findOneAndUpdate(
    { workerId: userId },
    { isAvailable },
    { new: true, runValidators: false }
  )
  if (!profile) {
    profile = await HamaliProfile.create({
      workerId: userId,
      isAvailable,
      currentLocation: {
        type: 'Point',
