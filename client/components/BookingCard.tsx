    ...
    <motion.div
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 20 }}>{isHamali ? '👷' : '🚛'}</span>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)' }}>{truncate(pickupAddr, 32)}</span>
        </div>
        {dropoffAddr && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Flag size={14} color="var(--teal)" style={{ flexShrink: 0 }} />
            <span style={{ fontSize: 14, color: 'var(--text-muted)' }}>{truncate(dropoffAddr, 32)}</span>
          </div>
        )}
      </div>
