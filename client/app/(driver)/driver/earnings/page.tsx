    ...
    >
      ...
      <motion.div variants={springPop} className="grid grid-cols-3 gap-3 mb-4">
        {summaryCards.map(({ label, value }) => (
          ...
          >
            <p className="font-syne font-800 text-lg" style={{ color: 'var(--accent)' }}>₹{value}</p>
      ...
      >
        ...
        <div>
          <p className="text-xs font-500" style={{ color: 'var(--accent)' }}>ALL TIME EARNINGS</p>
          <p className="font-syne font-800 text-2xl" style={{ color: 'var(--accent)' }}>₹{earnings?.allTime || 0}</p>
        </div>
      </motion.div>
      ...
      <motion.div variants={fadeUp}>
        <h3 className="font-syne font-700 text-base mb-3" style={{ color: 'var(--text)' }}>Completed Trips</h3>
        <div
          className="rounded-md overflow-hidden"
          style={{ border: '1px solid var(--border)' }}
        >
          {bookings.length === 0 ? (
            <div className="p-6 text-center" style={{ color: 'var(--text-muted)' }}>No completed trips yet</div>
          ) : bookings.map((b, i) => (
            <div
              key={b._id}
            ...
            >
              <div>
                ...
                <p className="text-sm font-500" style={{ color: 'var(--text)' }} title={b.pickup?.address + ' → ' + b.dropoff?.address}>
                  {b.pickup?.address?.substring(0, 20)}... → {b.dropoff?.address?.substring(0, 15)}...
                </p>
                <p className="text-xs" style={{ color: 'var(--text-faint)' }}>{b.bookingId}</p>
              </div>
