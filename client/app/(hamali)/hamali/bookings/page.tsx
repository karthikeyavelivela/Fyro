...
export default function HamaliBookings() {
  ...
  const fetchBookings = async () => {
    try {
      const { data } = await api.get('/api/hamali/bookings')
    ...
    <div className="fyro-page">
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 20 }}>
        {STATUSES.map(s => (
          <button
            key={s}
            onClick={() => setFilter(s)}
            className={`filter-pill${filter === s ? ' active-teal' : ''}`}
          >
      ...
      {!loading && !error && filtered.length === 0 && (
        <div style={{ background: 'var(--surface)', borderRadius: 16, border: '1px solid var(--border)', padding: '40px 24px', textAlign: 'center' }}>
          <Package size={32} color="var(--text-muted)" style={{ margin: '0 auto 12px', display: 'block' }} />
      ...
      {!loading && !error && filtered.map((b: any) => (
        <Link key={b._id} href={`/hamali/bookings/${b._id}`} style={{ textDecoration: 'none' }}>
          <div style={{
            ...
            onMouseEnter={e => { const t = e.currentTarget; t.style.borderColor = 'rgba(13,148,136,0.2)'; t.style.transform = 'translateX(3px)' }}
            onMouseLeave={e => { const t = e.currentTarget; t.style.borderColor = ''; t.style.transform = '' }}
          >
            <div style={{ width: 40, height: 40, borderRadius: 11, background: 'rgba(13,148,136,0.09)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <p style={{ fontFamily: 'Outfit, sans-serif', fontSize: 12, color: 'var(--text-muted)', margin: '2px 0 0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {b.workLocation?.address || b.pickup?.address || 'Hamali job'}
              </p>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, flexShrink: 0 }}>
              <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 15, color: 'var(--teal)' }}>
                ₹{b.finalFare || b.estimatedFare || 0}
              </span>
              <span style={{
                padding: '2px 8px', borderRadius: 999, fontSize: 10, fontWeight: 700,
                fontFamily: 'Outfit, sans-serif', textTransform: 'uppercase', letterSpacing: '0.05em',
                background: b.status === 'completed' ? 'rgba(22,163,74,0.1)' : b.status === 'cancelled' ? 'rgba(220,38,38,0.08)' : 'rgba(13,148,136,0.1)',
                color: b.status === 'completed' ? '#16A34A' : b.status === 'cancelled' ? '#DC2626' : 'var(--teal)'
              }}>
                {b.status?.replace('_', ' ')}
              </span>
            </div>
